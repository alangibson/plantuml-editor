...
const actions: any = {
  ...
  loadParams(context: any, macroParams: any) {
    ...
    if (macroParams.language) {
      context.dispatch('plantumlEditor/setLanguage', macroParams.language, {root: true})
    }
    // Recover document
    if (macroParams.doc) {
      console.debug(`[PowerPlantUML] Loading document. Length is ${macroParams.doc.length}`);
      context.dispatch('plantumlEditor/renderUML', macroParams.doc, {root: true})
      ...
      context.dispatch('plantumlEditor/renderEncodedUML', encodedUML, {root: true})
    }
    // TODO error?
